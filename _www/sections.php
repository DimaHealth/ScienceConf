...
$res = mysqli_query($connect, $sql);
  ...
  echo '<tr><th>ID</th><th>Секция</th><th>Мероприятие</th>
  <th></th><th></th></tr>'; 
  ...
  { 
    ...
    echo '<td><a href="?action=editform&id='.$item['IDSection'].'">Ред.</a></td>'; 
    echo '<td><a href="?action=delete&id='.$item['IDSection'].'">Удл.</a></td>'; 
    echo '</tr>'; 
  } 
  echo '</table>';
...
{ 
echo '<h2>Добавить</h2>';
include("templates/addSection.php");
...
{ 
  ...
  include("templates/updateSection.php");
} 
...
$id = mysqli_escape_string($connect, $_GET['IDSection'] );
  $Section = mysqli_escape_string($connect, $_POST['Section'] ); 
  $CodeEvent = mysqli_escape_string($connect, $_POST['CodeEvent'] ); 
  $query = "UPDATE sections SET Section='".$Section."', CodeEvent ='".$CodeEvent."' 
   WHERE IDSection=".$id;
