...
<head>
  ...
  <title>Включенные сотрудники</title>  
...
$sql = "SELECT * FROM involvedemployees";
require_once("dbconnect.php");
$res = mysqli_query($connect, $sql);

  echo '<h2>Сотрудники по мероприятиям</h2>'; 
  echo '<div class="col_66">';
    echo '<table border="1" class="table">';      
   
  echo '<tr><th>ID</th><th>Мероприятие</th><th>Сотрудник</th><th>Должность</th>
  <th></th><th></th></tr>'; 
  while ( $item = mysqli_fetch_array( $res ) ) 
  { 
    echo '<tr class="my-bold-font">';  
	echo '<td>'.$item['IDInvolvedEmployee'].'</td>'; 
	...
	$sql4 = "SELECT `Post` FROM `posts` WHERE IDPost = ".$item['CodePost'];
    $res4 =  mysqli_query($connect, $sql4);
    echo '<td>'.mysqli_fetch_array( $res4 )['Post'].'</td>';
	
    echo '<td><a href="?action=editform&id='.$item['IDInvolvedEmployee'].'">Ред.</a></td>'; 
    echo '<td><a href="?action=delete&id='.$item['IDInvolvedEmployee'].'">Удл.</a></td>'; 
    echo '</tr>'; 
  } 
  echo '</table>';
...
{ 
echo '<h2>Добавить</h2>'; 
include("templates/addInvolvedEmployee.php");
...
require_once("dbconnect.php");
 ...
 $query = " INSERT INTO `involvedemployees`(`CodeEvent`, `CodeEmployee`, `CodePost`) 
 VALUES ('$CodeEvent', '$CodeEmployee', '$CodePost')";
...
{ 
  ...
  $query = 'select * from involvedemployees WHERE IDInvolvedEmployee='.$id; 
  
  $res = mysqli_query($connect ,$query ); 
  $item = mysqli_fetch_array( $res ); 
  include("templates/updateInvolvedEmployee.php");
} 
...
{ 
  require_once("dbconnect.php");
  $id = mysqli_escape_string($connect, $_GET['IDInvolvedEmployee'] );
  $CodeEvent = mysqli_escape_string($connect, $_POST['CodeEvent'] ); 
  $CodeEmployee = mysqli_escape_string($connect, $_POST['CodeEmployee'] ); 
  $CodePost = mysqli_escape_string($connect, $_POST['CodePost'] ); 
  $query = "UPDATE involvedemployees SET CodeEvent ='".$CodeEvent
  ."', CodeEmployee ='".$CodeEmployee."', CodePost ='".$CodePost."' WHERE IDInvolvedEmployee=".$id;
...
{ 
  require_once("dbconnect.php");
  $id = empty($_GET["id"]) ? 0 : intval($_GET["id"]);
  $query = "DELETE FROM involvedemployees WHERE IDInvolvedEmployee=".$id; 
  mysqli_query ($connect, $query ); 
 // die(var_dump($_POST, $_GET, $id));
  header( 'Location: '.$_SERVER['PHP_SELF'] );
  die();
