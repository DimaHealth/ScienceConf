...
$res = mysqli_query($connect, $sql);
  ...
  echo '<h2>Должности</h2>'; 
  echo '<div class="col_66">';
    echo '<table border="1" class="table">';      
   
  echo '<tr><th>IDPost</th><th>Должность</th><th>Для мероприятия</th>
   <th>Ред.</th><th>Удл.</th></tr>'; 
  while ( $item = mysqli_fetch_array( $res ) ) 
  { 
    echo '<tr>'; 
	echo '<td>'.$item['IDPost'].'</td>'; 
    echo '<td>'.$item['Post'].'</td>'; 
    echo '<td>'.$item['ForEvent'].'</td>'; 
    echo '<td><a href="?action=editform&id='.$item['IDPost'].'">Ред.</a></td>'; 
    echo '<td><a href="?action=delete&id='.$item['IDPost'].'">Удл.</a></td>'; 
    echo '</tr>'; 
  } 
  echo '</table>';
...
require_once("dbconnect.php");
  ...
  $ForEvent = mysqli_escape_string($connect, $_POST['ForEvent'] ); 
  $query = " INSERT INTO `posts`(`Post`, `ForEvent`) 
 VALUES ('$Post',' $ForEvent')"; 
...
{ 
  ...
  include("templates/updatePost.php");
} 
...
{ 
require_once("dbconnect.php");
$id = mysqli_escape_string($connect, $_GET['IDPost'] );
  $Post = mysqli_escape_string($connect, $_POST['Post'] ); 
  $ForEvent = mysqli_escape_string($connect, $_POST['ForEvent'] ); 
  $query = "UPDATE posts SET Post='".$Post."', ForEvent ='".$ForEvent."' WHERE IDPost=".$id;
